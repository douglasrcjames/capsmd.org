import React, { Component } from 'react'

export default class Finding1MCPSDistributesStaffResourcesFightAchievementGap extends Component {
    render() {
        return (
            <div className="pdf-container">
                <iframe 
                    src="https://drive.google.com/file/d/1G1shMvUOsH-Z1an4ZSuCt-4q4HWkFyNF/preview" 
                    title="Finding 1: MCPS Distributes Staff Resources to Fight the Achievement Gap"
                    frameBorder="0" 
                    height="800px" 
                    width="100%">
                    <p>
                        This PDF could not be displayed, please download or view it 
                        <a href="https://drive.google.com/file/d/1G1shMvUOsH-Z1an4ZSuCt-4q4HWkFyNF/">
                            here.
                        </a>
                    </p>
                </iframe>
            </div>
        )
    }
}
